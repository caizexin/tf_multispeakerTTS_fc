...
from feedback_synthesizer.utils.text import text_to_sequence
...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, hparams):
		...
		with tf.device("/cpu:0"):
			...
			self.inputs, self.input_lengths, self.mel_targets, self.token_targets, \
				self.targets_lengths, self.split_infos, self.speaker_embeddings, self.embedding_masks = queue.dequeue()
			...
			self.eval_inputs, self.eval_input_lengths, self.eval_mel_targets, \
				...
				self.eval_split_infos, self.eval_speaker_embeddings, self.eval_embedding_masks = eval_queue.dequeue()
	...
	def _prepare_batch(self, batches, outputs_per_step):
		...
		return inputs, input_lengths, mel_targets, token_targets, targets_lengths, \
			   split_infos, embed_targets, embed_masks
	...
	def _prepare_embed_mask(self, masks, alignment):
	...
	def _pad_mask_target(self, m, length):
