...
class Feeder:
    ...
    def __call__(self, num_frames):
        ...
        # it would meet some parallel error regrading non-seekable files, use num_parallel_calls=1 or change all audio files into wave format
        # dataset = dataset.map(lambda spkid, wav_file:
        #                      tuple(tf.py_func(self._parse_func, [spkid, wav_file, num_frames], hp.dtypes)), num_parallel_calls=1)
...
class EvalFeeder:
    ...
    def __call__(self, num_frames):
        ...
        wav_names = dataset.map(lambda *row: row[1])
        wav_names = dataset.batch(hp.batch_size, drop_remainder=False)
        wav_names = dataset.make_initializable_iterator()
